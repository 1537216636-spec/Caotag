...
class _ItemFileImportState extends State<ItemFileImport> {
  ...
  Future<void> _importAccessory(
    ...
    // ---- 密钥提取 ----
    // 用于匹配报告的 hashedPublicKey 列表（keysMap 的所有值，共50个）
    ...
    // 用于查询的密钥（主私钥 + additionalKeys，共50个）
    final List<String> queryKeys = [];
    if (accessoryDTO.privateKey.isNotEmpty) {
      queryKeys.add(accessoryDTO.privateKey);
    }
    if (rawJson['additionalKeys'] != null) {
      for (var key in rawJson['additionalKeys']) {
        if (key is String && !queryKeys.contains(key)) {
          queryKeys.add(key);
        }
      }
    }
    ...
    Accessory newAccessory = Accessory(
      ...
      name: accessoryDTO.name.isNotEmpty ? accessoryDTO.name : (rawJson['id_short']?.toString() ?? accessoryDTO.id.toString()),
      color: color,
      icon: icon,
